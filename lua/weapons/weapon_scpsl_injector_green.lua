...
if CLIENT then 
    SWEP.WepSelectIcon = surface.GetTextureID( "vgui/hud/injector_green" )
...
function SWEP:Deploy()
	...
	if owner:GetAmmoCount(self.Primary.Ammo) == 0 then owner:StripWeapon("weapon_scpsl_injector_green") end
...
local function Heal(owner, weapon)
    ...
    if IsValid(weapon) then
        if IsValid(owner) and SERVER and activeWeapon:GetClass() == "weapon_scpsl_injector_green" then
        
			if InitializeSEF == true then
				    owner:ApplyEffect("Energized", 1, 5, 1)
				    owner:ApplyEffect("Healing", 1, 5, 1)
					owner:ApplyEffect("Endurance", 60)
...
function SWEP:SecondaryAttack()
    ...
    if self.InitializeHealing == 1 then
        ...
        if trace.HitPos then
            local ENT = ents.Create("weapon_scpsl_injector_green")
            if IsValid(ENT) then
	    ...
	    owner:RemoveAmmo(1, "injector")
	    if owner:GetAmmoCount(self.Primary.Ammo) == 0 then owner:StripWeapon("weapon_scpsl_injector_green") end -- Reminder
	end
end
