if CLIENT then 
    SWEP.WepSelectIcon = surface.GetTextureID( "vgui/hud/medkit_blue" )
	SWEP.BounceWeaponIcon = true 
    SWEP.DrawWeaponInfoBox = true
end

SWEP.PrintName = "Charger Kit"
SWEP.Author = "Craft_Pig"
SWEP.Purpose = [[
Regenerates 30 Health and Armor over 5 seconds.
Cures Bleeding
]]
...
function SWEP:Deploy()
	...
	timer.Simple(0.05, function()
        if IsValid(self) and IsValid(self.Owner) then
            ...
            if IsValid(vm) then
                vm:SetSkin(1) -- Change this to 1 if you want the second texture group
            end
        end
    end)
	...
	if owner:GetAmmoCount(self.Primary.Ammo) == 0 then owner:StripWeapon("weapon_scpsl_medkit_blue") end -- Reminder
...
local function Heal(owner, weapon)
    ...
    if IsValid(weapon) then
        if IsValid(owner) and SERVER and activeWeapon:GetClass() == "weapon_scpsl_medkit_blue" then -- Reminder
		
		    if InitializeSEF == true then
			    owner:ApplyEffect("Healing", 5, 3, 0.5)
				owner:ApplyEffect("Energized", 5, 3, 0.5)
...
function SWEP:SecondaryAttack()
    ...
    if self.InitializeHealing == 1 then
        ...
        if trace.HitPos then
            local ENT = ents.Create("weapon_scpsl_medkit_blue")
            if IsValid(ENT) then
